(function(root,factory){
  ...
  function render(result,title='Краткие рекомендации по результатам'){
    ...
    return `<details class="fd-recommendations" open><summary><div><span class="tag">Рекомендации</span><h3>${escapeHtml(title)}</h3></div><i aria-hidden="true">⌄</i></summary><div class="fd-rec-body"><div class="fd-rec-verdict"><b>Краткий вывод</b><p>${escapeHtml(model.summary)}</p></div><div class="fd-rec-list">${items}</div><section class="fd-rec-consult"><div><span class="tag">Персональный разбор</span><h4>Нужен более точный план действий?</h4><p>Экспресс-расчёт показывает только основные сигналы. На консультации можно детально разобрать обязательства, сроки платежей, дебиторскую задолженность, структуру расходов и причины текущей ситуации в компании.</p></div><a class="btn primary" href="../contacts/">Обратиться за консультацией</a></section><div class="notice fd-rec-disclaimer"><b>Важно:</b> рекомендации сформированы автоматически по введённым данным и не заменяют аудит или индивидуальное финансовое заключение.</div></div></details>`;
  }
